...
1. 難易度を選択（初級:5件 / 上級:7件）してゲーム開始
2. 初回ゲーム開始時に表示されるチュートリアルを確認（以後はヘルプボタンで再表示可能）
3. 表示された動画サムネイルをドラッグ&ドロップ、または各カードの ▲▼ ボタンで並び替え
4. 公開日が古い順に並び替えて「回答をチェック」をクリック
5. 結果が表示されます（正解率、各動画の正誤、公開日）

##  操作の補足

- ヘルプ表示中はタイマーが停止し、閉じると再開します。
- 端末によってはドラッグ操作が不安定になる場合があります。その際は ▲▼ ボタンでの並び替えを利用してください。
...
│   │   └── TutorialModal.tsx # ヘルプ/チュートリアルモーダル
│   ├── utils/              # ユーティリティ関数
